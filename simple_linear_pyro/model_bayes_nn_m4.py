...
def model_fn(nn_model):
    ...
    def _model(x_data, y_data):
        ...
        fc1w_prior = Normal(torch.zeros(1,2,3), torch.ones(1,2,3)).to_event(2)
        fc1b_prior = Normal(0.25*torch.ones(1,2), 0.25*torch.ones(1,2)).to_event(1)

        outw_prior = Normal(torch.zeros(1,1,2), torch.ones(1,1,2)).to_event(2)
        outb_prior = Normal(0.25*torch.ones(1,1), 0.25*torch.ones(1,1)).to_event(1)
