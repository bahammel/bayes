...
def model_fn(nn_model):
    ...
    def _model(x_data, y_data):
        ...
        fc1w_prior = Normal(torch.zeros(1,400,100), torch.ones(1,400,100)).to_event()
        fc1b_prior = Normal(10*torch.ones(1,400), 10*torch.ones(1,400)).to_event()

        outw_prior = Normal(torch.zeros(1,1,400), torch.ones(1,1,400)).to_event()
        outb_prior = Normal(10*torch.ones(1,1), 10*torch.ones(1,1)).to_event()
...
if __name__ == '__main__':
    nn_model = NN_Model(input_size=(1,100), hidden_size=(1,400), output_size=1)
    model = model_fn(nn_model)
    guide = guide_fn(nn_model)
