...
def model_fn(nn_model):
    ...
    def _model(x_data, y_data):
        ...
        fc1w_prior = Normal(torch.zeros(400,100), torch.ones(400,100)).to_event()
        fc1b_prior = Normal(1*torch.ones(400), 2*torch.ones(400)).to_event()

        outw_prior = Normal(torch.zeros(1,400), torch.ones(1,400)).to_event()
        outb_prior = Normal(1*torch.ones(1), 2*torch.ones(1)).to_event()
...
def guide_fn(nn_model):
    ...
    def _guide(x_data, y_data):
        ...
        sigma_loc = pyro.param('sigma_loc', torch.tensor(50.0),
                             constraint=constraints.positive)
        sigma = pyro.sample("sigma", Normal(sigma_loc, torch.tensor(10.0)))
