...
def pyromodel(x, y):
    ...
    otp_uncertainty = [20,20,20,2000,200,20,20,20,20,20,20]
    sigma = pyro.sample('sigma', Uniform(0, torch.Tensor(otp_uncertainty)))
...
def guide(x, y):
    ...
    for name, par in model.named_parameters():
        ...
        scale = softplus(pyro.param(name + '.scale',
                                    -3.0 * torch.ones(*par.shape) + 0.1 * torch.randn(*par.shape)))
    ...
    sigma_loc = pyro.param('sigma_loc', torch.randn(11),
