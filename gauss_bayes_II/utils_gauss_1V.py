...
def gauss_data_2():
    ...
    for cases in range(num_of_cases):
        ...
        mu = np.random.choice(np.linspace(0.6, 1.4, 1000))
        ...
        for gauss in range(num_of_gauss):
            ...
            I = I + amp * (1.0 - hu/12.0) / (2*std*np.sqrt(2.*np.pi)) * np.exp(-(hu - mult*10.*mu)**2./(2.*mult*(2*std)**2.))
        #Y.append([mu,std,amp])
        Y.append([mu])
        X.append(I)
    Y = np.array(Y)
    ...
    idx_mu_min = np.where(Y[:,0] == Y[:,0].min())
    idx_mu_max = np.where(Y[:,0] == Y[:,0].max())
    plt.figure()
    plt.title('data')
    plt.plot(hu,X[idx_mu_min][0,:], label='mu_min')
    plt.plot(hu,X[idx_mu_max][0,:], label='mu_max')
    plt.legend()
