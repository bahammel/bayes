...
def gauss_data_2():
    ...
    for cases in range(num_of_cases):
        ...
        mu = np.random.choice(np.linspace(0.3, 1.0, 100))
        std = np.random.choice(np.linspace(0.45, 0.55, 100))
        #std = 0.5
        ...
        for gauss in range(num_of_gauss):
            ...
            I = I + amp * (1.0 - hu/12.0) / (2*std*np.sqrt(2.*np.pi)) * np.exp(-(hu - mult*20*mu)**2./(2.*mult*(2*std)**2.))
        #Y.append([mu,std,amp])
        Y.append([mu,std])
        X.append(I)
    Y = np.array(Y)
    ...
    idx_mu_min_std_min = np.where((Y[:,0] == Y[:,0].min()) & (Y[:,1] == Y[:,1].min()))
    idx_mu_min_std_max = np.where((Y[:,0] == Y[:,0].min()) & (Y[:,1] == Y[:,1].max()))
    idx_mu_max_std_min = np.where((Y[:,0] == Y[:,0].max()) & (Y[:,1] == Y[:,1].min()))
    idx_mu_max_std_max = np.where((Y[:,0] == Y[:,0].max()) & (Y[:,1] == Y[:,1].max()))
    plt.figure()
    plt.title('data')
    plt.plot(hu,X[idx_mu_min_std_min][0,:], label='mu_min_std_min')
    plt.plot(hu,X[idx_mu_min_std_max][0,:], label='mu_min_std_max')
    plt.plot(hu,X[idx_mu_max_std_min][0,:], label='mu_max_std_min')
    plt.plot(hu,X[idx_mu_max_std_max][0,:], label='mu_max_std_max')
    plt.legend()
...
def gauss_data_bay():
    ...
    for cases in range(num_of_cases):
        ...
        mu = 5.5
        std = np.random.choice(np.linspace(0.5, 2.0, 2))
        #std = 0.5
#        amp = np.random.choice(np.linspace(0.1, 1.0,20))
        amp = 0.5
        #Add up num_of_gauss different Gaussians, where mu and std are scaled by mult, to make fake spectrum
...
#    print(X)
    ...
    plt.figure()
    plt.title('data')
    plt.plot(hu,np.transpose(xtrain[::50]))
    """
    return hu, xtrain, ytrain, xtest, ytest, X, Y
