...
def plot_pred(df):
    plt.figure()
    plt.errorbar(df['y_test'], df['pred_mean'], yerr=df['pred_std'], fmt='o', c='b', ms = 1, elinewidth=1.0,errorevery=1)  #mean of predicted values w 1std error
    plt.legend()


def plot_obs(df):
    plt.figure()
    plt.errorbar(df['y_test'], df['obs_mean'], yerr=df['obs_std'], fmt='+', c='r', ms = 2, elinewidth=1.5,errorevery=1)  #mean of predicted values w 1std error
    plt.errorbar(df['y_test'], df['pred_mean'], yerr=df['pred_std'], fmt='o', c='b', ms = 1, elinewidth=1.0,errorevery=1)  #mean of predicted values w 1std error
    plt.legend()
...
if __name__ == '__main__':
    ...
    ys =  np.zeros(shape=(125000,nsamples,1))
