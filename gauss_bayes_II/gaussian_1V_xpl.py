...
def pyromodel(x, y):
    ...
    for name, par in model.named_parameters():
        priors[name] = dist.Normal(torch.zeros(1,*par.shape), 10 * torch.ones(1,*par.shape)).independent(par.dim())

        print("prior batch shape:", priors[name].batch_shape)
        print("prior event shape:", priors[name].event_shape)
        print("prior event dim:", priors[name].event_dim)    
...
def guide(x, y):
    dists = {}
    for name, par in model.named_parameters():
        loc = pyro.param(name + '.loc', torch.randn(1,*par.shape))
        scale = softplus(pyro.param(name + '.scale',
                                    -3.0 * torch.ones(1,*par.shape) + 0.05 * torch.randn(1,*par.shape)))
