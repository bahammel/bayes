...
model = torch.nn.Sequential(
    torch.nn.Linear(33, hidden1_size),
    torch.nn.Sigmoid(),
    torch.nn.Linear(hidden1_size, hidden2_size),
    torch.nn.Sigmoid(),
    torch.nn.Linear(hidden2_size, 1),
)
...
plt.errorbar(xs[:,1,np.newaxis].cpu().numpy(), mean, yerr=std, fmt='o', c='g',label='pred mean')  #mean of predicted values w 1std error
