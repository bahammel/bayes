...
def plot_pred(df):
    plt.figure()
    plt.errorbar(df['y_test'], df['pred_mean'], yerr=df['pred_std'], fmt='o', c='b', ms = 3, elinewidth=1.0,errorevery=1)  #mean of predicted values w 1std error
    plt.legend()


def plot_obs(df):
    plt.figure()
    plt.errorbar(df['y_test'], df['obs_mean'], yerr=df['obs_std'], fmt='+', c='r', ms = 7, elinewidth=3.0,errorevery=1)  #mean of predicted values w 1std error
    plt.errorbar(df['y_test'], df['pred_mean'], yerr=df['pred_std'], fmt='o', c='b', ms = 4, elinewidth=1.0,errorevery=1)  #mean of predicted values w 1std error
    plt.legend()
...
if __name__ == '__main__':
    ...
    plt.xscale('linear')
    plt.yscale('linear')
    plt.title("Fit to y[1]")
    plt.xlabel("y_test")
    plt.ylabel("y_pred")
    plt.errorbar(ytest[:,1], mean_1, yerr=std_1, fmt='o', c='r', ms = 1, elinewidth=0.1,errorevery=1)  #mean of predicted values w 1std error
