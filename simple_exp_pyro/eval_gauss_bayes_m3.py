...
from data_exp_bayes import get_dataset, seed_everything
import glob
...
def guide_summary(guide, x_data, y_data):
    ...
    df = pd.DataFrame({
        ...
        "pred_perc_5": pred_5q[idx],
        "pred_perc_95": pred_95q[idx],
    })
    plot_pred(df)
...
def plot_pred(df):
    ...
    plt.plot(df['Index'], df['pred_mean'], 'o', color='C1', label='pred')
